...
TypeLookupTable TypeLookupTable::Create(const DexFile& dex_file, uint8_t* storage /* = nullptr */) {
  return TypeLookupTable(dex_file, storage);
}

TypeLookupTable TypeLookupTable::Open(const uint8_t* dex_file_pointer,
                                      const uint8_t* raw_data,
                                      uint32_t num_class_defs) {
  return TypeLookupTable(dex_file_pointer, raw_data, num_class_defs);
}
...
uint32_t TypeLookupTable::Lookup(const char* str, uint32_t hash) const {
  ...
  do {
    DCHECK(!entry->IsLast(mask_bits_));
    pos = (pos + entry->GetNextPosDelta(mask_bits_)) & mask;
    entry = &entries_[pos];
    DCHECK(!entry->IsEmpty());
...
TypeLookupTable::TypeLookupTable(const DexFile& dex_file, uint8_t* storage)
    : dex_data_begin_(dex_file.DataBegin()),
      ...
      entries_(SupportedSize(dex_file.NumClassDefs())
                   ? (storage != nullptr)
                       ? reinterpret_cast<Entry*>(storage)
                       : new Entry[1u << mask_bits_]
                   : nullptr),
      owned_entries_(storage == nullptr ? entries_ : nullptr) {
  static_assert(alignof(Entry) == 4u, "Expecting Entry to be 4-byte aligned.");
  DCHECK_ALIGNED(storage, alignof(Entry));
  const uint32_t mask = Entry::GetMask(mask_bits_);
  ...
  for (size_t class_def_idx = 0; class_def_idx < dex_file.NumClassDefs(); ++class_def_idx) {
    ...
    if (entries_[pos].IsEmpty()) {
      entries_[pos] = Entry(str_id.string_data_off_, hash, class_def_idx, mask_bits_);
      DCHECK(entries_[pos].IsLast(mask_bits_));
    } else {
  ...
  for (uint16_t class_def_idx : conflict_class_defs) {
    ...
    DCHECK(!entries_[tail_pos].IsEmpty());
    while (!entries_[tail_pos].IsLast(mask_bits_)) {
      tail_pos = (tail_pos + entries_[tail_pos].GetNextPosDelta(mask_bits_)) & mask;
      DCHECK(!entries_[tail_pos].IsEmpty());
    }
    ...
    } while (!entries_[insert_pos].IsEmpty());
    // Insert and chain the new entry.
    entries_[insert_pos] = Entry(str_id.string_data_off_, hash, class_def_idx, mask_bits_);
    entries_[tail_pos].SetNextPosDelta((insert_pos - tail_pos) & mask, mask_bits_);
    DCHECK(entries_[insert_pos].IsLast(mask_bits_));
    DCHECK(!entries_[tail_pos].IsLast(mask_bits_));
  }
}
