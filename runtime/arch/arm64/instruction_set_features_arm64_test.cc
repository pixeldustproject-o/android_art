...
TEST(Arm64InstructionSetFeaturesTest, Arm64Features) {
  ...
  std::unique_ptr<const InstructionSetFeatures> cortex_a53_features(
      InstructionSetFeatures::FromVariant(InstructionSet::kArm64, "cortex-a53", &error_msg));
  ASSERT_TRUE(cortex_a53_features.get() != nullptr) << error_msg;
  EXPECT_EQ(cortex_a53_features->GetInstructionSet(), InstructionSet::kArm64);
  ...
  EXPECT_EQ(cortex_a53_crc_features->GetInstructionSet(), InstructionSet::kArm64);
